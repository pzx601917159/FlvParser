...
int FdkAACDecoder::decodeFrame(unsigned char* frameData, unsigned int frameSize,
        int* width, int* height, int* pixFmt, int pts)
{
    printf("fdkAacDecoder decode frame\n");
    ...
    while(1)
    {
        ...
        adts_header_t* adts =
            reinterpret_cast<adts_header_t*>(&aacBuf_[0] + pos_);
        ...
        if(ret == AAC_DEC_OK)
        {
            ...
            // SDLPlayer::instance()->playAudio((unsigned char*)&(pcm_buf[0]),
            //        validSize);
            BinaryData* data = new BinaryData();
            data->size_ = validSize;
            data->data_ = reinterpret_cast<unsigned char*>(malloc(validSize));
            memcpy(data->data_, &pcm_buf[0], validSize);
            //VideoState::audioFrames_.emplace(data);
        }
        else
        {
...
int FdkAACDecoder::aacdec_fill(char *data, int nb_data, int *pnb_left)
{
    ...
    AAC_DECODER_ERROR err = aacDecoder_Fill(_h.dec, &udata,
            &unb_data, &unb_left);
    if (err != AAC_DEC_OK)
    {
    ...
    if(pnb_left)
    {
        *pnb_left = static_cast<int>(unb_left);
    }
...
int FdkAACDecoder::aacdec_pcm_size()
{
    ...
    return static_cast<int>(_h.info->numChannels * _h.info->frameSize *
            _h.sample_bits / 8);
}
...
int FdkAACDecoder::aacdec_decode_frame(char *pcm, int nb_pcm, int *pnb_valid)
{
    ...
    INT_PCM* upcm = reinterpret_cast<INT_PCM*>(pcm);
    int unb_pcm = static_cast<int>(nb_pcm);
